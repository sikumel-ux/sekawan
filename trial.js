...
// FETCH DATA MASTER UTAMA (GET METHOD)
async function ambilDataZSheets() {
    try {
        ...
        if(json.status === "success") {
            ...
            tampilAlert("Gagal", "Sistem gagal sinkronisasi database pusat.", false);
            return false;
        }
    } catch(e) {
        console.error(e);
...
function bukaDashboardWarga() {
    ...
    // 1. DISTRIBUSI PEMBAYARAN IURAN WARGA
    ...
    // --- DETEKSI DINAMIS BULAN BERGABUNG ---
    ...
    // --- RENDER FOTO PROFIL (DEFAULT avatar.png) ---
    ...
    // 3. CETAK GRID STATUS IURAN BULANAN
...
// --- ⚙️ PROSES FIX TOTAL: UPLOAD FOTO PROFIL (PURE JSON POST) ---
async function uploadFotoProfil(input) {
    ...
    reader.onload = async function(e) {
        ...
        try {
            // Pengiriman murni JSON Object, anti-pecah dan anti-gagal dibaca Apps Script
            let dataPaket = {
                action: "updateFoto",
            ...
            if(json.status === "success" || json.url) {
                ...
                tampilAlert("Gagal", "Server Apps Script gagal menyimpan gambar.", false);
            }
        } catch(err) {
            console.error(err);
            tampilAlert("Error", "Gagal mengunggah foto profil ke cloud server.", false);
        } finally {
            hideLoading();
        }
    };
...
// --- ⚙️ PROSES FIX TOTAL: GANTI PASSWORD (PURE JSON POST) ---
async function prosesGantiPassword() {
    ...
    try {
        ...
        if(json.status === "success") {
            ...
            tampilAlert("Gagal", json.message || "Gagal merubah password di database.", false);
        }
    } catch(e) {
        console.error(e);
        tampilAlert("Error", "Gagal terhubung merubah password.", false);
    } finally {
        hideLoading();
    }
}
...
// MANAGEMENT AUTO LOGIN KETIKA WARGA REFRESH BROWSER
