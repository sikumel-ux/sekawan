...
// FUNGSI AMBIL DATA UTAMA DARI SHEET PUSAT
...
// RE-WRITE SINKRONISASI DATASHEET KE PORTAL
function bukaDashboardWarga() {
    ...
    // --- 🛠️ RE-FIX TOTAL: FORMAT ULANG STRIP BULAN BERGABUNG AGAR SINKRON ---
    let txtBergabung = "JANUARI 2025"; 
    let mentahBergabung = null;
    
    // Tarik nilai key dinamis dari row record database user
    ...
    if (mentahBergabung) {
        ...
        if (!isNaN(objekTgl.getTime())) {
            ...
            // Jika dikirim string format tanggal "YYYY-MM-DD" oleh Apps Script
            ...
            // Jika isian di Google Sheet berupa ketikan teks langsung (misal: "Agustus 2025")
    ...
    // --- 🛠️ RE-FIX TOTAL: DETEKSI PROPERTI FOTO PROFIL ---
    const avatar = document.getElementById('avatarWarga');
    let urlFoto = null;
    for (let key in sessionWarga) {
        if (key.toLowerCase().replace(/\s+/g, '') === 'foto') {
            urlFoto = sessionWarga[key];
            break;
        }
    }
    if(!urlFoto) {
...
// --- 🛠️ RE-WRITE FINAL: PROSES POST FOTO MENGGUNAKAN FORM DATA MULTIPART ---
async function uploadFotoProfil(input) {
    ...
    reader.onload = async function(e) {
        ...
        try {
            // Kita bungkus ke dalam Object FormData murni (Sangat disukai oleh sistem backend Google Drive/GAS API)
            let dataForm = new FormData();
            dataForm.append('action', 'updateFoto');
            dataForm.append('hp', sessionWarga.hp.toString());
            dataForm.append('filename', file.name);
            dataForm.append('mimeType', file.type);
            dataForm.append('fileData', base64Data);

            // Tembak langsung menggunakan payload FormData murni tanpa stringify parameters
            ...
            if(json.status === "success" || json.url || json.fileUrl) {
                let linkBaru = json.url || json.fileUrl || json.videoUrl;
                
                // Cari properti key foto di lokal lalu inject nilainya
                ...
                tampilAlert("Gagal", "Server gagal menyimpan berkas foto profil.", false);
            }
        } catch(err) {
            console.error(err);
            tampilAlert("Error", "Gagal terhubung mengunggah foto profil.", false);
        } finally {
            hideLoading();
        }
    };
