...
// Keadaan Listener Status Sampah (untuk mencegah pemanggilan ganda)
...
function bukaDashboardWarga() {
    ...
    // RENDERING FORMAT TANGGAL GABUNG
    ...
    // =========================================================================
    // FITUR MONITORING STATUS SAMPAH WARGA (OTOMATIS CEK JADWAL BESOK)
    // =========================================================================
    const iconBox = document.getElementById('iconStatusSampah');
    const txtStatus = document.getElementById('txtStatusSampah');
    const tglStatus = document.getElementById('tglStatusSampah');
    const lblJadwal = document.getElementById('labelJadwalSampah');

    // Pengaturan Jadwal Patokan Utama RT
    lblJadwal.innerText = "Rabu & Sabtu (08:00 - 10:00)";

    if(statusSampahRef) statusSampahRef.off();

    statusSampahRef = dbFirebase.ref('status_sampah/' + sessionWarga.hp);
    statusSampahRef.on('value', (snapshot) => {
        ...
        let statusSekarang = data ? data.status : "Kosong";
        let waktuUpdate = data ? data.waktu_update : "Belum ada catatan hari ini";

        // Tentukan info hari esok atau jadwal terdekat secara dinamis
        let jadwalBerikutnya = "Rabu";
        if (hariIni === "Rabu" || hariIni === "Kamis" || hariIni === "Jumat") {
            jadwalBerikutnya = "Sabtu";
        } else if (hariIni === "Sabtu") {
            jadwalBerikutnya = "Rabu";
        } else if (hariIni === "Selasa") {
            jadwalBerikutnya = "Besok (Rabu)";
        } else if (hariIni === "Jumat") {
            jadwalBerikutnya = "Besok (Sabtu)";
        }

        // Filter: Jika BUKAN hari Rabu atau Sabtu, paksa view kembali Kosong + Info Jadwal
        if (hariIni !== "Rabu" && hariIni !== "Sabtu") {
            statusSekarang = "Kosong";
            waktuUpdate = `Petugas libur hari ini. Jadwal berikutnya: Hari ${jadwalBerikutnya}`;
        } else {
            // Jika HARI H (Rabu/Sabtu) tapi data Firebase ternyata sisa minggu lalu, paksa reset ke Kosong
            if (data && data.waktu_update && !data.waktu_update.includes(hariIni)) {
                statusSekarang = "Kosong";
                waktuUpdate = "Petugas belum keliling. Menunggu update hari ini...";
            }
        }

        txtStatus.innerText = statusSekarang;
        tglStatus.innerText = waktuUpdate;

        if (statusSekarang === "Diambil") {
            iconBox.className = "w-12 h-12 rounded-xl flex items-center justify-center bg-emerald-50 text-emerald-600 shadow-xs";
            iconBox.innerHTML = '<span class="material-symbols-rounded !text-2xl">delete_check</span>';
            txtStatus.className = "text-sm font-black uppercase tracking-tight text-emerald-600";
        } else if (statusSekarang === "Tidak Diambil") {
            iconBox.className = "w-12 h-12 rounded-xl flex items-center justify-center bg-rose-50 text-rose-500 shadow-xs";
            iconBox.innerHTML = '<span class="material-symbols-rounded !text-2xl">delete_forever</span>';
            txtStatus.className = "text-sm font-black uppercase tracking-tight text-rose-500";
        } else {
            iconBox.className = "w-12 h-12 rounded-xl flex items-center justify-center bg-slate-100 text-slate-400 shadow-xs";
            iconBox.innerHTML = '<span class="material-symbols-rounded !text-2xl">delete</span>';
            txtStatus.className = "text-sm font-black uppercase tracking-tight text-slate-400";
        }
    });

    // =========================================================================
    // VALIDASI & RENDER PANEL KHUSUS PETUGAS SAMPAH
    // =========================================================================
...
// FUNGSI KHUSUS PETUGAS: UPDATE STATUS KE FIREBASE
function updateStatusSampah(statusPilihan) {
    ...
    const teksWaktu = `${namaHari[sekarang.getDay()]}, ${jamMenit} WIB`;

    dbFirebase.ref('status_sampah/' + hpWargaTerpilih).update({
        status: statusPilihan,
        waktu_update: `Diupdate: ${teksWaktu}`
    }).then(() => {
        hideLoading();
        tampilAlert("Status Terkirim", `Berhasil mengubah status sampah warga menjadi: ${statusPilihan}`, true);
    }).catch((err) => {
        console.error(err);
        hideLoading();
        tampilAlert("Gagal", "Gagal mengirim data status sampah ke Firebase.", false);
    });
}
