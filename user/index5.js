...
// SYSTEM SWITCH TAB ROUTER MULTI TAB
...
function salinRekening() {
    ...
    navigator.clipboard.writeText(rek).then(() => {
        tampilAlert("Berhasil Copas", "Nomor rekening berhasil disalin ke papan klip!", true);
    });
}
...
async function ambilDataZSheets() {
    try {
        ...
        if(json.status === "success") {
            ...
            tampilAlert("Gagal", "Sistem gagal mengambil data pusat.", false);
            return false;
        }
    } catch(e) {
        console.error(e);
    ...
    if(cocok) {
        dbFirebase.ref('users/' + inputHp).once('value').then((snapshot) => {
            ...
            if (inputPass === passwordValid) {
                ...
                tampilAlert("Akses Ditolak", "Kata Sandi yang dimasukkan salah, bro!", false);
            }
            hideLoading();
        }).catch((err) => {
            console.error(err);
...
// INTI MATRIKS DISTRIBUSI KONTEN MODUL DATA
function bukaDashboardWarga() {
    ...
    // Sinkronisasi data identitas atas
    ...
    // 1. Sedot Data Pembayaran Iuran Warga
    ...
    // 2. Sedot Data Kas Umum Operasional
    if(dbGlobal.kas) {
    ...
    // RENDERING TAB 1: CARD LAPORAN KAS MAKRO KESELURUHAN (TANPA POP UP)
    ...
    // =========================================================================
    // FITUR REAL-TIME KALENDER MONITORING SAMPAH BULANAN (TAB 2: DATA)
    // =========================================================================
    ...
    // BACKDOOR PANEL INTEGRASI PETUGAS
    ...
    // 3. Cetak Checklist Kisi Bulanan Iuran (TAB 2: DATA)
    ...
    // 4. Cetak Histori Pembayaran Pribadi (TAB 2: DATA)
    ...
    // 5. Cetak Laporan Mutasi Kas Gabungan Terbuka (TAB 1: DASHBOARD - TANPA POP UP)
...
// RENDERING KALENDER DI TAB DATA (1-31)
    ...
    for (let hari = 1; hari <= jumlahHari; hari++) {
        ...
        if (rincian) {
            if (rincian.status === "Diambil") {
                kelasWarna = "bg-emerald-600 text-white font-black shadow-2xs active:scale-90";
            } else if (rincian.status === "Tidak Diambil") {
                kelasWarna = "bg-rose-500 text-white font-black shadow-2xs active:scale-90";
            }
        }
...
function prosesGantiPasswordFirebase() {
    ...
    dbFirebase.ref('users/' + sessionWarga.hp).update({
        ...
        hideLoading();
        tampilAlert("Sandi Diperbarui", "Kata sandi sukses diubah di server Firebase, bro!", true);
    }).catch(() => {
        hideLoading();
