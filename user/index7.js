...
function kirimKritikSaranWA() {
    if(!sessionWarga) return;
    const teks = `Halo Admin TUNTAS, saya ${sessionWarga.nama.toUpperCase()} ingin menyampaikan kritik & saran masukan program:\n\n- `;
    window.open(`https://wa.me/${NOMOR_ADMIN_WHATSAPP}?text=${encodeURIComponent(teks)}`, '_blank');
}
...
function salinRekening() {
    ...
    navigator.clipboard.writeText(rek).then(() => {
        tampilAlert("Berhasil Copas", "Nomor rekening berhasil disalin!", true);
    });
}
...
async function ambilDataZSheets() {
    try {
        ...
        if(json.status === "success") {
            ...
            tampilAlert("Gagal", "Gagal mengambil data pusat.", false);
            return false;
        }
    } catch(e) {
        console.error(e);
        tampilAlert("Putus Koneksi", "Gagal terhubung ke server.", false);
        return false;
    }
}
...
async function prosesLoginWarga() {
    ...
    if(!inputHp || !inputPass) {
        tampilAlert("Form Kosong", "Nomor HP dan Kata Sandi wajib diisi!", false);
        return;
    }
    ...
    if(cocok) {
        dbFirebase.ref('users/' + inputHp).once('value').then((snapshot) => {
            ...
            if (inputPass === passwordValid) {
                ...
                tampilAlert("Akses Ditolak", "Kata Sandi salah, bro!", false);
            }
            hideLoading();
        }).catch((err) => {
            console.error(err);
            hideLoading();
            tampilAlert("Error", "Gagal memvalidasi database.", false);
        });
    } else {
        hideLoading();
        tampilAlert("Akses Ditolak", "Nomor WhatsApp belum terdaftar.", false);
    }
}
...
function bukaDashboardWarga() {
    ...
    document.getElementById('infoNamaUser').innerText = namaUserUpper;
    ...
    if(dbGlobal.pembayaran) {
        dbGlobal.pembayaran.forEach(d => {
            ...
            if(namaPenyetor === namaUserUpper) {
                textKeterangan = (d.keterangan || '').toString().trim();
    ...
    document.getElementById('dashPemasukanKas').innerText = 'Rp ' + totalPemasukan.toLocaleString('id-ID');
    document.getElementById('dashPengeluaranKas').innerText = 'Rp ' + totalPengeluaran.toLocaleString('id-ID');
    document.getElementById('dashSisaSaldoKas').innerText = 'Rp ' + saldoKasRealTime.toLocaleString('id-ID');
    ...
    document.getElementById('infoGabungUser').innerText = txtBergabung.toUpperCase();
    ...
    // =========================================================================
    // INITIALIZATION & REAL-TIME LISTENER MONITORING SAMPAH (FIXED BLANK HARIS)
    // =========================================================================
    ...
    // PERBAIKAN: Gambar grid 1-31 secara instan duluan agar tidak kosong/blank putih
    dataSampahBulanIni = {};
    gambarGridKalender(saiki.getFullYear(), saiki.getMonth());

    // Hubungkan ke listener firebase untuk sinkronisasi warna
    ...
    statusSampahRef.on('value', (snapshot) => {
        ...
        // Gambar ulang untuk memperbarui warna hijau/merah real-time saat ada data masuk
        gambarGridKalender(saiki.getFullYear(), saiki.getMonth());
    });
    ...
    daftarBulan.forEach(bln => {
        ...
        if(isLunas) {
            gridBulan.insertAdjacentHTML('beforeend', `
                <div class="p-3 bg-emerald-600 border border-emerald-700 rounded-2xl text-center text-white flex flex-col items-center justify-center min-h-[54px]">
    ...
    if(riwayatIuranPribadi.length === 0) {
        ...
        sortedPribadi.forEach(tx => {
            ...
            listPribadi.insertAdjacentHTML('beforeend', `
                <div class="bg-white p-4 rounded-2xl flex justify-between items-center border border-slate-100 shadow-2xs">
    ...
    // Log Mutasi Kas Gabungan Terbuka di Dashboard
    const listMutasiDash = document.getElementById('listMutasiKasDashboard');
    listMutasiDash.innerHTML = '';
    if(mutasiKasGabungan.length === 0) {
        listMutasiDash.innerHTML = '<p class="text-center text-xs text-slate-400 py-4">Data mutasi kas kosong.</p>';
    } else {
        ...
        sortedMutasi.forEach(m => {
            ...
            listMutasiDash.insertAdjacentHTML('beforeend', `
                <div class="py-3 flex justify-between items-center text-[11px]">
                    <div class="max-w-[70%]">
...
// RENDERING GRID KALENDER STABIL ANGKA MURNI 1-31
function gambarGridKalender(tahun, bulan) {
    const container = document.getElementById('gridAngkaKalender');
    container.innerHTML = ''; 

    // Cari batas maksimal jumlah hari riil di bulan berjalan (28, 30, atau 31 hari)
    const jumlahHari = new Date(tahun, bulan + 1, 0).getDate();

    // Loop murni berurutan dari tanggal 1 sampai batas akhir bulan
    for (let hari = 1; hari <= jumlahHari; hari++) {
        let rincian = dataSampahBulanIni[hari];
        
        // Warna Default: Abu-abu (Belum ada catatan atau Kosong)
        let kelasWarna = "bg-slate-100 border border-slate-200 text-slate-600 font-bold hover:bg-slate-200";
        
        if (rincian) {
            if (rincian.status === "Diambil") {
                kelasWarna = "bg-emerald-600 text-white font-black shadow-md active:scale-90";
            } else if (rincian.status === "Tidak Diambil") {
                kelasWarna = "bg-rose-500 text-white font-black shadow-md active:scale-90";
            }
        }

        container.insertAdjacentHTML('beforeend', `
            <button onclick="klikDetailTanggalSampah(${hari})" class="${kelasWarna} py-3 rounded-2xl text-xs transition-all focus:outline-none">
                ${hari}
            </button>
        `);
    }
...
// MANAGEMENT DETEKSI KLIK POP UP DETAIL DATA OPERASIONAL JAM PETUGAS
function klikDetailTanggalSampah(hari) {
    ...
    if (dataDay && dataDay.status !== "Kosong") {
        txtStatus.innerText = dataDay.status.toUpperCase();
        txtJam.innerText = `Waktu Angkut: ${dataDay.waktu || 'Tidak Tercatat'}`;
        ...
        txtStatus.innerText = "BELUM ADA DATA";
        txtStatus.className = "text-base font-black uppercase tracking-tight mt-1 text-slate-400";
        txtJam.innerText = "Petugas belum menginput data atau armada libur pada tanggal ini, bro.";
    }
...
function prosesGantiPasswordFirebase() {
    ...
    if(!passBaru) {
        tampilAlert("Gagal", "Sandi baru tidak boleh kosong!", false);
        return;
    }
    ...
    dbFirebase.ref('users/' + sessionWarga.hp).update({
        ...
        hideLoading();
        tampilAlert("Error", "Gagal menyimpan password baru.", false);
    });
}
    ...
    reader.onload = function(e) {
        ...
        img.onload = function() {
            ...
            dbFirebase.ref('users/' + sessionWarga.hp).update({
                ...
                hideLoading();
                tampilAlert("Gagal Upload", "Gagal mengunggah foto.", false);
            });
        };
    };
...
function updateStatusSampah(statusPilihan) {
    ...
    if (statusPilihan === "Kosong") {
        targetNode.remove().then(() => {
            hideLoading();
            tampilAlert("Data Direset", "Catatan sampah warga dikosongkan.", true);
        }).catch(() => {
            hideLoading();
